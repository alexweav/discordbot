defmodule DiscordBot.Gateway do
  ...
  def start_link(opts) do
    ...
    Supervisor.start_link(__MODULE__, {url, shard_count, broker_sup, broker}, opts)
  end

  def init({url, shard_count_arg, broker_sup, broker}) do
    ...
    children =
      ...
      ] ++ gateway_specs(token, url, broker, broker_sup, shard_count)
  ...
  defp gateway_specs(token, url, broker, broker_sup, shard_count) do
    for idx <- 0..(shard_count - 1),
        do: gateway_sup_spec(token, url, broker, broker_sup, idx, shard_count)
  end

  defp gateway_sup_spec(token, url, broker, broker_sup, shard_index, shard_count) do
